...
import (
	...
	kubeadmutil "github.com/carlory/firefly/pkg/adm/util"
)
...
func GetAPIServerAltNames(client kubernetes.Interface, cfg *kubeadmapi.InitConfiguration) (*certutil.AltNames, error) {
	if client == nil {
		return nil, errors.New("kubernetes client is nil")
	}

	internalAPIServerVirtualIP, err := constants.GetAPIServerVirtualIP(cfg.Networking.ServiceSubnet)
	if err != nil {
	...
	lbAltNames, err := getAPIServerLoadBalanceAltNames(client, cfg)
	if err != nil {
		return nil, errors.Wrap(err, "unable to get load balancer alt names")
	}
	altNames.DNSNames = append(altNames.DNSNames, lbAltNames.DNSNames...)
	altNames.IPs = append(altNames.IPs, lbAltNames.IPs...)
	...
	appendSANsToAltNames(altNames, cfg.APIServer.CertSANs, constants.APIServerCertName)

	return altNames, nil
}

func getAPIServerLoadBalanceAltNames(client kubernetes.Interface, cfg *kubeadmapi.InitConfiguration) (*certutil.AltNames, error) {
	altNames := &certutil.AltNames{}
	var appendNodeAltNames = func() error {
		...
		for _, node := range nodes.Items {
			for _, addr := range node.Status.Addresses {
				...
				case v1.NodeInternalIP, v1.NodeExternalIP:
					altNames.IPs = append(altNames.IPs, netutils.ParseIPSloppy(addr.Address))
				default:
					altNames.DNSNames = append(altNames.DNSNames, addr.Address)
				}
			}
		}
		return nil
	...
	lbsvc, err := client.CoreV1().Services(cfg.FireflyNamespace).Get(context.TODO(), "kube-apiserver", metav1.GetOptions{})
	if err != nil {
		return nil, errors.Wrapf(err, "unable to get service kube-apiserver in namespace %s", cfg.FireflyNamespace)
	}

	altNames.DNSNames = append(altNames.DNSNames, []string{
		"kube-apiserver",
		fmt.Sprintf("kube-apiserver.%s", cfg.FireflyNamespace),
		fmt.Sprintf("kube-apiserver.%s.svc", cfg.FireflyNamespace),
		fmt.Sprintf("kube-apiserver.%s.svc.%s", cfg.FireflyNamespace, cfg.Networking.DNSDomain),
	}...)

	switch lbsvc.Spec.Type {
	case v1.ServiceTypeClusterIP:
		altNames.IPs = append(altNames.IPs, netutils.ParseIPSloppy(lbsvc.Spec.ClusterIP))
	case v1.ServiceTypeNodePort:
		altNames.IPs = append(altNames.IPs, netutils.ParseIPSloppy(lbsvc.Spec.ClusterIP))
		if err := appendNodeAltNames(); err != nil {
			return nil, err
		}
	case v1.ServiceTypeLoadBalancer:
		if lbsvc.Spec.LoadBalancerIP != "" {
			altNames.IPs = append(altNames.IPs, netutils.ParseIPSloppy(lbsvc.Spec.LoadBalancerIP))
		}
		for _, ingress := range lbsvc.Status.LoadBalancer.Ingress {
			if ingress.IP != "" {
				altNames.IPs = append(altNames.IPs, netutils.ParseIPSloppy(ingress.IP))
			}
			if ingress.Hostname != "" {
				altNames.DNSNames = append(altNames.DNSNames, ingress.Hostname)
			}
		}
	}
	return altNames, nil
...
func GetEtcdAltNames(client kubernetes.Interface, cfg *kubeadmapi.InitConfiguration) (*certutil.AltNames, error) {
	return getAltNames(cfg, constants.EtcdServerCertName)
}
...
func GetEtcdPeerAltNames(client kubernetes.Interface, cfg *kubeadmapi.InitConfiguration) (*certutil.AltNames, error) {
	return getAltNames(cfg, constants.EtcdPeerCertName)
}
...
func getAltNames(cfg *kubeadmapi.InitConfiguration, certName string) (*certutil.AltNames, error) {
	...
	// create AltNames with defaults DNSNames/IPs
	altNames := &certutil.AltNames{
		// Fixme: this is a workaround for etcd
		DNSNames: []string{
			"localhost",
			"etcd",
			fmt.Sprintf("etcd.%s", cfg.FireflyNamespace),
			fmt.Sprintf("etcd.%s.svc", cfg.FireflyNamespace),
			fmt.Sprintf("etcd.%s.svc.%s", cfg.FireflyNamespace, cfg.Networking.DNSDomain),
			"etcd-0",
		},
	...
	if cfg.Etcd.Local != nil {
		if certName == constants.EtcdServerCertName {
			appendSANsToAltNames(altNames, cfg.Etcd.Local.ServerCertSANs, constants.EtcdServerCertName)
		} else if certName == constants.EtcdPeerCertName {
			appendSANsToAltNames(altNames, cfg.Etcd.Local.PeerCertSANs, constants.EtcdPeerCertName)
		}
	}
	return altNames, nil
}
