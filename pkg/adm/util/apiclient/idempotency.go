...
// CreateOrUpdatePersistentVolumeClaim creates a Claim if the target resource doesn't exist. If the resource exists already, this function will update the resource instead.
func CreateOrUpdatePersistentVolumeClaim(client clientset.Interface, claim *v1.PersistentVolumeClaim) error {
	got, err := client.CoreV1().PersistentVolumeClaims(claim.ObjectMeta.Namespace).Get(context.TODO(), claim.ObjectMeta.Name, metav1.GetOptions{})
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return errors.Wrap(err, "unable to get persistentvolumeclaim")
		}
		_, err := client.CoreV1().PersistentVolumeClaims(claim.ObjectMeta.Namespace).Create(context.TODO(), claim, metav1.CreateOptions{})
		return errors.Wrap(err, "unable to create persistentvolumeclaim")
	}
	got.Spec.Resources.Requests = claim.Spec.Resources.Requests.DeepCopy()
	_, err = client.CoreV1().PersistentVolumeClaims(got.ObjectMeta.Namespace).Update(context.TODO(), got, metav1.UpdateOptions{})
	return errors.Wrap(err, "unable to update persistentvolumeclaim")
}
