...
type configMutatorsFunc func(kubernetes.Interface, *kubeadmapi.InitConfiguration, *pkiutil.CertConfig) error
...
func (k *KubeadmCert) GetConfig(client kubernetes.Interface, ic *kubeadmapi.InitConfiguration) (*pkiutil.CertConfig, error) {
	for _, f := range k.configMutators {
		if err := f(client, ic, &k.config); err != nil {
			return nil, err
		}
	}
...
func (t CertificateTree) CreateTree(client kubernetes.Interface, ic *kubeadmapi.InitConfiguration) error {
	for ca, leaves := range t {
		cfg, err := ca.GetConfig(client, ic)
		if err != nil {
			return err
		}
		...
		for _, leaf := range leaves {
			if err := leaf.CreateFromCA(client, ic, caCert, caKey); err != nil {
				return err
			}
		}
	}
	...
	return &KubeadmCert{
		...
		config: pkiutil.CertConfig{
			Config: certutil.Config{
				...
				Organization: []string{constants.SystemPrivilegedGroup},
...
func makeAltNamesMutator(f func(kubernetes.Interface, *kubeadmapi.InitConfiguration) (*certutil.AltNames, error)) configMutatorsFunc {
	return func(client kubernetes.Interface, mc *kubeadmapi.InitConfiguration, cc *pkiutil.CertConfig) error {
		altNames, err := f(client, mc)
		if err != nil {
			return err
		}
...
func createKeyAndCSR(client kubernetes.Interface, kubeadmConfig *kubeadmapi.InitConfiguration, cert *KubeadmCert) error {
	...
	cfg, err := cert.GetConfig(client, kubeadmConfig)
	if err != nil {
		return err
	}
...
func CreateDefaultKeysAndCSRFiles(out io.Writer, client kubernetes.Interface, config *kubeadmapi.InitConfiguration) error {
	...
	for _, cert := range certificates {
		if err := createKeyAndCSR(client, config, cert); err != nil {
			return err
		}
		if out != nil {
			fmt.Fprintf(out, "  %s\n", cert.BaseName)
