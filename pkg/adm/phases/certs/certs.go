...
import (
	...
	"github.com/carlory/firefly/pkg/adm/util/pkiutil"
)
...
func CreatePKIAssets(client kubernetes.Interface, cfg *kubeadmapi.InitConfiguration) error {
	...
	if err := certTree.CreateTree(client, cfg); err != nil {
...
func CreateServiceAccountKeyAndPublicKeyFiles(certsDir string, keyType x509.PublicKeyAlgorithm) error {
	...
	_, err := keyutil.PrivateKeyFromFile(filepath.Join(certsDir, constants.ServiceAccountPrivateKeyName))
	if err == nil {
		...
		fmt.Printf("[certs] Using the existing %q key\n", constants.ServiceAccountKeyBaseName)
		return nil
	} else if !os.IsNotExist(err) {
		return errors.Wrapf(err, "file %s existed but it could not be loaded properly", constants.ServiceAccountPrivateKeyName)
	}
	...
	if err := pkiutil.WriteKey(certsDir, constants.ServiceAccountKeyBaseName, key); err != nil {
		return err
	}

	return pkiutil.WritePublicKey(certsDir, constants.ServiceAccountKeyBaseName, key.Public())
}
...
func CreateCACertAndKeyFiles(client kubernetes.Interface, certSpec *KubeadmCert, cfg *kubeadmapi.InitConfiguration) error {
	...
	certConfig, err := certSpec.GetConfig(client, cfg)
	if err != nil {
		return err
	}
...
func CreateCSR(client kubernetes.Interface, certSpec *KubeadmCert, cfg *kubeadmapi.InitConfiguration, path string) error {
	csr, key, err := NewCSR(client, certSpec, cfg)
	if err != nil {
		return err
	}
...
func CreateCertAndKeyFilesWithCA(client kubernetes.Interface, certSpec *KubeadmCert, caCertSpec *KubeadmCert, cfg *kubeadmapi.InitConfiguration) error {
	...
	return certSpec.CreateFromCA(client, cfg, caCert, caKey)
}
...
func SharedCertificateExists(cfg *kubeadmapi.ClusterConfiguration) (bool, error) {
	var errs []error
	if err := validateCACertAndKey(certKeyLocation{cfg.CertificatesDir, constants.CACertAndKeyBaseName, "", "CA"}); err != nil {
		errs = append(errs, err)
	}

	if err := validatePrivatePublicKey(certKeyLocation{cfg.CertificatesDir, "", constants.ServiceAccountKeyBaseName, "service account"}); err != nil {
		errs = append(errs, err)
	}

	if err := validateCACertAndKey(certKeyLocation{cfg.CertificatesDir, constants.FrontProxyCACertAndKeyBaseName, "", "front-proxy CA"}); err != nil {
		errs = append(errs, err)
	}

	// in case of local/stacked etcd
	if cfg.Etcd.External == nil {
		if err := validateCACertAndKey(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, "", "etcd CA"}); err != nil {
			errs = append(errs, err)
		}
	}
	if len(errs) != 0 {
...
func UsingExternalEtcdCA(cfg *kubeadmapi.ClusterConfiguration) (bool, error) {
	if err := validateCACert(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, "", "etcd CA"}); err != nil {
		return false, err
	}

	path := filepath.Join(cfg.CertificatesDir, constants.EtcdCAKeyName)
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		return false, nil
	}

	if err := validateSignedCert(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, constants.APIServerEtcdClientCertAndKeyBaseName, "apiserver etcd client"}); err != nil {
		return true, err
	}

	if err := validateSignedCert(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, constants.EtcdServerCertAndKeyBaseName, "etcd server"}); err != nil {
		return true, err
	}

	if err := validateSignedCert(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, constants.EtcdPeerCertAndKeyBaseName, "etcd peer"}); err != nil {
		return true, err
	}

	if err := validateSignedCert(certKeyLocation{cfg.CertificatesDir, constants.EtcdCACertAndKeyBaseName, constants.EtcdHealthcheckClientCertAndKeyBaseName, "etcd health-check client"}); err != nil {
		return true, err
	}
