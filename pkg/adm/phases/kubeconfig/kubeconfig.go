...
func getKubeConfigSpecs(cfg *kubeadmapi.InitConfiguration) (map[string]*kubeConfigSpec, error) {
	caCert, caKey, err := pkiutil.TryLoadCertAndKeyFromDisk(cfg.CertificatesDir, constants.CACertAndKeyBaseName)
	...
	// Validate period
	certsphase.CheckCertificatePeriodValidity(constants.CACertAndKeyBaseName, caCert)
...
func WriteKubeConfigWithClientCert(out io.Writer, cfg *kubeadmapi.InitConfiguration, clientName string, organizations []string, notAfter *time.Time) error {
	...
	controlPlaneEndpoint, err := kubeadmutil.GetControlPlaneEndpoint(cfg.ControlPlaneEndpoint)
	if err != nil {
		return err
	}
...
func WriteKubeConfigWithToken(out io.Writer, cfg *kubeadmapi.InitConfiguration, clientName, token string, notAfter *time.Time) error {
	...
	// creates the KubeConfigSpecs, actualized for the current InitConfiguration
	caCert, _, err := pkiutil.TryLoadCertAndKeyFromDisk(cfg.CertificatesDir, constants.CACertAndKeyBaseName)
	if err != nil {
		return errors.Wrap(err, "couldn't create a kubeconfig; the CA files couldn't be loaded")
	}
	// Validate period
...
func getKubeConfigSpecsBase(cfg *kubeadmapi.InitConfiguration) (map[string]*kubeConfigSpec, error) {
	...
	inClusterAPIEndpoint := "https://kube-apiserver:6443"
	controlPlaneEndpoint := "https://kube-apiserver:6443"
	var err error
	...
	return map[string]*kubeConfigSpec{
		constants.AdminKubeConfigFileName: {
			...
			ClientCertAuth: &clientCertAuth{
				Organizations: []string{constants.SystemPrivilegedGroup},
			},
		},
		constants.ControllerManagerKubeConfigFileName: {
			APIServer:      inClusterAPIEndpoint,
