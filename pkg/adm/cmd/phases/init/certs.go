...
var (
	saKeyLongDesc = fmt.Sprintf(cmdutil.LongDesc(`
		...
		`+cmdutil.AlphaDisclaimer), constants.ServiceAccountPrivateKeyName, constants.ServiceAccountPublicKeyName)
...
func newCertSubPhase(certSpec *certsphase.KubeadmCert, run func(c workflow.RunData) error) workflow.Phase {
	phase := workflow.Phase{
		...
		Long: fmt.Sprintf(
			...
			certSpec.LongName,
			certSpec.BaseName,
			getSANDescription(nil, certSpec),
		),
		Run:          run,
...
func getSANDescription(client kubernetes.Interface, certSpec *certsphase.KubeadmCert) string {
	...
	certConfig, err := certSpec.GetConfig(client, defaultInternalConfig)
	if err != nil {
		return ""
	}
...
func runCAPhase(ca *certsphase.KubeadmCert) func(c workflow.RunData) error {
	return func(c workflow.RunData) error {
		...
		if cert, err := pkiutil.TryLoadCertFromDisk(data.CertificateDir(), ca.BaseName); err == nil {
			...
			if data.DryRun() {
				err := fireflyadmutil.CopyFile(filepath.Join(data.CertificateDir(), constants.CACertName), filepath.Join(data.CertificateWriteDir(), constants.CACertName))
				if err != nil {
					return errors.Wrapf(err, "could not copy %s to dry run directory %s", constants.CACertName, data.CertificateWriteDir())
				}
			}
			if _, err := pkiutil.TryLoadKeyFromDisk(data.CertificateDir(), ca.BaseName); err == nil {
				...
				if data.DryRun() {
					err := fireflyadmutil.CopyFile(filepath.Join(data.CertificateDir(), constants.CAKeyName), filepath.Join(data.CertificateWriteDir(), constants.CAKeyName))
					if err != nil {
						return errors.Wrapf(err, "could not copy %s to dry run directory %s", constants.CAKeyName, data.CertificateWriteDir())
					}
				}
		...
		return certsphase.CreateCACertAndKeyFiles(client, ca, cfg)
	}
}
...
func runCertPhase(cert *certsphase.KubeadmCert, caCert *certsphase.KubeadmCert) func(c workflow.RunData) error {
	return func(c workflow.RunData) error {
		...
		return certsphase.CreateCertAndKeyFilesWithCA(client, cert, caCert, cfg)
	}
}
