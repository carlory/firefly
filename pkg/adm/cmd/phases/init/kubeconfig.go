...
import (
	...
	kubeconfigphase "github.com/carlory/firefly/pkg/adm/phases/kubeconfig"
...
func NewKubeConfigPhase() workflow.Phase {
	return workflow.Phase{
		...
		Phases: []workflow.Phase{
			...
			NewKubeConfigFilePhase(constants.AdminKubeConfigFileName),
			NewKubeConfigFilePhase(constants.ControllerManagerKubeConfigFileName),
			NewKubeConfigFilePhase(constants.SchedulerKubeConfigFileName),
		},
		Run: runKubeConfig,
	}
}
...
func runKubeConfigFile(kubeConfigFileName string) func(workflow.RunData) error {
	return func(c workflow.RunData) error {
		...
		if data.ExternalCA() {
			...
			if data.DryRun() {
				err := fireflyadmutil.CopyFile(filepath.Join(constants.KubernetesDir, kubeConfigFileName), filepath.Join(data.KubeConfigDir(), kubeConfigFileName))
				if err != nil {
